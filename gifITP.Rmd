...
```{r, echo=FALSE,out.width="100%"}
  Mapplot <- plotmap(ITA)+
    ...
    transition_states(data,
                      transition_length = 1,
                      state_length = 0.5,wrap = FALSE) +labs(title = "Made with R by Jürgen Mayr, Date:{frame_time}", x="Longitude",y="Latitude",colour="Casi totali") + theme(plot.title = element_text(size=20), legend.text = element_text( size = 20), text = element_text( size = 20)) + theme(legend.title = element_text( size=20))+ guides(fill = guide_colourbar(barwidth = 2, barheight = 30))+
