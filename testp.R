...
titletext <- paste0("Data Source:COVID19 github repository Umberto Rosini.  Provinz  Bozen <br />  Date : ", Sys.Date(), "<br /> Created with: R by J. Mayr")
...
leaflet(tilesURL) %>% 
  ...
  addPolygons(stroke = F,fillOpacity = .7,fillColor = ~pal(pcmdpcr$totale_casi/pcmdpcr$residenti*100),labelOptions = labelOptions(textsize = "15px",opacity = 0.7),label = lapply(paste0("<b>",pcmdpcr$provincia,"</b> <br />  in Positive in %: ",formatC( pcmdpcr$totale_casi/pcmdpcr$residenti*100),"<br />  Fälle: ",pcmdpcr$totale_casi ), htmltools::HTML))%>%
